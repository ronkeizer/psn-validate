...
EOF
    ...
    $help_text{-prepend_model_to_lst} = <<'EOF';
    <p class="style2">-prepend_model_to_lst</p>
...
unless ( defined $options{'skip_data_parsing'} 
	 or $options{'drop_dropped'}
	 or $options{'extra_data_files'}
	 or $options{'wrap_data'}){
...
if ( defined $options{'predict_data'} and defined $options{'predict_model'} ) {
  if( scalar @{$models_array} > 1 ) {
    debug -> die( message => "When using predict_data and predict_model, no "
